...
# 全局状态存储
_reading_states: Dict[str, ReadingState] = {}
_jieba_module = None
...
def load_reading_states(state_file: str = "novel_reading_state.json") -> None:
    """加载读取状态"""
    global _reading_states
    if os.path.exists(state_file):
        try:
            with open(state_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                for file_path, state_dict in data.items():
                    _reading_states[file_path] = ReadingState.from_dict(state_dict)
        except Exception as e:
            print(f"加载读取状态失败: {e}")
            _reading_states = {}


def save_reading_states(state_file: str = "novel_reading_state.json") -> None:
    """保存读取状态"""
    try:
        data = {file_path: state.to_dict() for file_path, state in _reading_states.items()}
...
def find_sentence_boundary(text: str, target_position: int) -> int:
    """在目标位置附近找到完整的句子边界"""
...
def init_novel_file(file_path: str, chunk_size: int = 500, state_file: str = "novel_reading_state.json") -> ReadingState:
    """初始化小说文件读取状态"""
    global _reading_states
    ...
    # 加载状态
    load_reading_states(state_file)
    ...
    # 创建或更新读取状态
    if file_path in _reading_states:
        state = _reading_states[file_path]
        ...
        _reading_states[file_path] = state
    
    save_reading_states(state_file)
    return state


def read_next_chunk(file_path: str, overlap_sentences: int = 1, state_file: str = "novel_reading_state.json") -> Tuple[str, bool, Dict[str, Any]]:
    """
    读取下一个文本块
    ...
    if file_path not in _reading_states:
    ...
    try:
        with open(file_path, 'r', encoding=state.encoding) as f:
            ...
            if len(raw_text) == buffer_size:
                # 还没到文件末尾，需要找句子边界
                boundary_pos = find_sentence_boundary(raw_text, state.chunk_size)
...
def reset_reading_position(file_path: str, state_file: str = "novel_reading_state.json") -> bool:
    """重置文件读取位置到开头"""
    global _reading_states
    ...
    if file_path in _reading_states:
        _reading_states[file_path].current_position = 0
        save_reading_states(state_file)
        return True
    return False


def set_reading_position(file_path: str, position: int, state_file: str = "novel_reading_state.json") -> bool:
    """设置文件读取位置"""
    global _reading_states
...
def get_reading_progress(file_path: str, state_file: str = "novel_reading_state.json") -> Optional[Dict[str, Any]]:
    """获取读取进度信息"""
    global _reading_states
...
def list_reading_states(state_file: str = "novel_reading_state.json") -> Dict[str, Dict[str, Any]]:
    """列出所有文件的读取状态"""
    global _reading_states
    ...
    result = {}
    for file_path in _reading_states.keys():
        progress = get_reading_progress(file_path, state_file)
